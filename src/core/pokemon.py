...
class Pokemon:
    def __init__(self, nom, type_, pv_max, attaque, defense, special, vitesse, attaques_possibles=None):
        ...
        self.attaques_possibles = attaques_possibles if attaques_possibles is not None else []
        self.attaques = []
        self.stat_stages = {"attaque": 0, "defense": 0, "special": 0, "vitesse": 0}
        self.statut = None # 'PAR', 'BRU', 'PSN', 'SOM', 'GEL' ou None
    ...
    def modify_stat(self, stat_name, amount):
        if stat_name not in self.stat_stages: return False
        old_stage = self.stat_stages[stat_name]
        self.stat_stages[stat_name] = max(-6, min(6, old_stage + amount))
        return self.stat_stages[stat_name] != old_stage
